...
export async function GET() {
  try {
    ...
    await prisma.company.createMany({
      data: [
        { name: 'Innovate Inc.', industryId: industries.find(i => i.name === 'Technology')!.id, regionId: regions.find(r => r.name === 'San Francisco Bay Area')!.id },
        { name: 'DataDriven Corp.', industryId: industries.find(i => i.name === 'Technology')!.id, regionId: regions.find(r => r.name === 'New York City')!.id },
        { name: 'FinanceFirst Bank', industryId: industries.find(i => i.name === 'Finance')!.id, regionId: regions.find(r => r.name === 'New York City')!.id },
        { name: 'HealthWell Group', industryId: industries.find(i => i.name === 'Healthcare')!.id, regionId: regions.find(r => r.name === 'Austin, TX')!.id },
      ],
    });
